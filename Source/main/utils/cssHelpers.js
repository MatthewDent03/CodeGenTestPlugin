...
export function getStroke(node) {
    if ("strokes" in node &&
        ...
        if (paint && paint.type === "SOLID") {
            ...
            const green = Math.round(paint.color.g * 255);
            const blue = Math.round(paint.color.b * 255);
            const position = node.strokeAlign || "CENTER";
            return {
...
export function getPaddingCSS(node) {
    if (!("padding" in node))
        return "";
...
export function getBorderRadiusCSS(node) {
...
export function getFlexGapCSS(node) {
...
export function getOpacityCSS(node) {
    if (!("opacity" in node))
        return "";
...
export function getShadowCSS(node) {
    var _a, _b;
    if (!("effects" in node))
        return "";
