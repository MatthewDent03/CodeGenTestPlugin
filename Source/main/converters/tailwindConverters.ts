...
export function pxToLetterSpacing(px: number, fontSize: number = 16): string {
  if (px === 0) return "";
...
export function pxToSpacing(px: number, prefix: string = "gap"): string {
  if (px === 0) return "";
  const tailwindClass = findClosest(px, spacingMap);
  if (tailwindClass) return `${prefix}-${tailwindClass}`;
  return `${prefix}-[${px}px]`;
...
export function pxToPosition(px: number, prefix: string = "left"): string {
  if (px === 0) return "";
  return `${prefix}-[${px}px]`;
}
