...
function inferTokenType(path) {
    ...
    if (category === "typography") {
        if (subcategory === "fontFamily")
            return "fontFamily";
        if (subcategory === "fontWeight")
            return "fontWeight";
        if (subcategory === "lineHeight")
            return "number";
        return "dimension";
    }
    if (category === "effects") {
        if (subcategory === "opacity")
            return "number";
...
function toW3CTokens(node, path = []) {
...
export class TokenRegistry {
     ...
     */
    getToken(category, colorName, shade) {
        // Navigate to the color group: tokens[category][colorName]
        const categoryData = this.tokenData[category];
        if (!categoryData) {
            return null;
        }
    ...
    getShades(category, colorName) {
        ...
        if (typeof colorGroup !== "object" ||
            (colorGroup && "$value" in colorGroup)) {
            return [];
        }
     ...
     */
    getSpacingToken(tokenName) {
        const spacingData = this.tokenData.spacing;
        if (!spacingData) {
            return null;
        }
     ...
     */
    getTypographyToken(category, tokenName) {
        const typographyData = this.tokenData.typography;
        if (!typographyData) {
            return null;
        }
     ...
     */
    getEffectsToken(category, tokenName) {
        const effectsData = this.tokenData.effects;
        if (!effectsData) {
            return null;
        }
